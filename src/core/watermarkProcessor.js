...
const OUTLINE_REFINEMENT_THRESHOLD = 0.25;
const OUTLINE_REFINEMENT_MIN_GAIN = 1.2;
const SUBPIXEL_REFINE_SHIFTS = [-0.5, -0.25, 0, 0.25, 0.5];
const SUBPIXEL_REFINE_SCALES = [0.98, 0.99, 1, 1.01, 1.02];
const ALPHA_GAIN_CANDIDATES = [1.02, 1.05, 1.08, 1.12, 1.16, 1.2, 1.24, 1.28, 1.32, 1.36, 1.40, 1.45, 1.52, 1.6, 1.7, 1.85, 2.0, 2.2, 2.4, 2.6];
...
const PREVIEW_EDGE_CLEANUP_MAX_APPLIED_PASSES = 5;
...
function shouldStopAfterFirstPass({
    ...
    if (Math.abs(firstPassSpatialScore) <= 0.12) {
        return true;
    }
